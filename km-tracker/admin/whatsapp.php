...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'salvar_config') {
        ...
        if ($exists) {
            $db->prepare("UPDATE evolution_config SET evolution_url=?, evolution_key=?, instance_name=?, admin_phone=?,
                notif_novo_evento=?, notif_presenca_confirmada=?, notif_lembrete=?, notif_enquete=? WHERE id=?")
               ->execute([$url, $key, $instance, $phone, $n_evento, $n_pres, $n_lemb, $n_enq, $exists]);
        } else {
            $db->prepare("INSERT INTO evolution_config (evolution_url, evolution_key, instance_name, admin_phone,
                notif_novo_evento, notif_presenca_confirmada, notif_lembrete, notif_enquete) VALUES (?,?,?,?,?,?,?,?)")
               ->execute([$url, $key, $instance, $phone, $n_evento, $n_pres, $n_lemb, $n_enq]);
        }
    ...
    if ($action === 'criar_notificacao') {
        ...
        if ($titulo && $mensagem) {
            $db->prepare("INSERT INTO notificacoes (tipo, titulo, mensagem, destinatario, user_id, event_id, agendado_para, status, criado_por) VALUES (?,?,?,?,?,?,?,?,?)")
               ->execute([$tipo, $titulo, $mensagem, $dest, $userId, $eventId, $agendado, 'pendente', $me['id']]);
        ...
        if ($titulo && $pergunta && count($opcoes) >= 2) {
            ...
            if ($enviarNow) {
                ...
                try {
                    ...
                    $_SESSION['flash_success'] = 'Enquete criada e enviada no WhatsApp!';
                } catch (Throwable $e) {
                    $_SESSION['flash_success'] = 'Enquete criada. Erro no WhatsApp: ' . $e->getMessage();
                }
            } else {
...
<!-- Tabs -->
<div style="display:flex;gap:4px;margin-bottom:24px;background:#14161c;border:1px solid #2a2f3a;border-radius:10px;padding:5px;width:fit-content">
    <?php
    $tabs = ['config'=>'⚙️ Configuração','notificacoes'=>'🔔 Notificações','enquetes'=>'📊 Enquetes','logs'=>'📋 Logs'];
    foreach ($tabs as $t => $l):
    ?>
    <a href="?tab=<?= $t ?>" style="padding:8px 18px;border-radius:7px;font-size:.82rem;font-weight:600;text-decoration:none;transition:all .15s;
        <?= $tab===$t ? 'background:#f39c12;color:#0d0f14;' : 'color:#6e7485;' ?>">
        <?= $l ?>
    </a>
    <?php endforeach; ?>
</div>
...
.wpp-form-group label { display:block;font-size:.8rem;font-weight:600;color:#a0a5b5;margin-bottom:6px; }
.wpp-form-group input,
.wpp-form-group select,
.wpp-form-group textarea {
    width:100%;background:#0d0f14;border:1px solid #2a2f3a;border-radius:7px;
    padding:9px 12px;color:#eef0f8;font-size:.85rem;font-family:inherit;
    outline:none;transition:border-color .15s;
}
.wpp-form-group input:focus,
.wpp-form-group select:focus,
.wpp-form-group textarea:focus { border-color:#f39c12; }
.wpp-form-group small { font-size:.72rem;color:#6e7485;margin-top:4px;display:block; }
...
.notif-row { padding:14px 20px;border-bottom:1px solid #2a2f3a; }
        ...
        <div class="card-body">
            <form method="POST">
                ...
                <div style="margin-bottom:16px">
                    <div style="font-size:.8rem;font-weight:600;color:#a0a5b5;margin-bottom:10px">Notificações automáticas</div>
                    <?php foreach ([
                    ...
                    <label style="display:flex;align-items:center;gap:10px;margin-bottom:8px;cursor:pointer;font-size:.85rem;color:#a0a5b5">
        ...
        <div class="card" style="margin-bottom:20px">
            ...
            <div class="card-body" style="text-align:center;padding:28px 20px">
                <?php if ($connStatus === 'unconfigured'): ?>
                    ...
                    <div style="font-weight:700;color:#6e7485">Não configurado</div>
                    <div style="font-size:.78rem;color:#6e7485;margin-top:4px">Preencha e salve as configurações</div>
                <?php elseif ($connStatus === 'open'): ?>
                    <div style="font-size:2.5rem;margin-bottom:10px">✅</div>
                    <div style="font-weight:700;color:#28a745;font-size:1.1rem">Conectado</div>
                    <div style="font-size:.78rem;color:#6e7485;margin-top:4px">WhatsApp ativo e pronto para enviar</div>
                <?php elseif ($connStatus === 'error'): ?>
                    <div style="font-size:2.5rem;margin-bottom:10px">❌</div>
                    <div style="font-weight:700;color:#dc3545">Erro de conexão</div>
                    <div style="font-size:.78rem;color:#6e7485;margin-top:4px">Verifique a URL e a API Key</div>
                <?php else: ?>
                    <div style="font-size:2.5rem;margin-bottom:10px">📱</div>
                    <div style="font-weight:700;color:#f39c12;font-size:1rem">Desconectado</div>
                    <div style="font-size:.78rem;color:#6e7485;margin:8px 0 16px">Escaneie o QR Code com o WhatsApp do admin</div>
                    <?php if ($qrCode): ?>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
                    <div style="text-align:center;padding:14px;background:#0d0f14;border-radius:8px;border:1px solid #2a2f3a">
                        <div style="font-size:1.6rem;font-weight:700;color:#28a745"><?= $comWpp ?></div>
                        <div style="font-size:.72rem;color:#6e7485;margin-top:2px">Com WhatsApp</div>
                    </div>
                    <div style="text-align:center;padding:14px;background:#0d0f14;border-radius:8px;border:1px solid #2a2f3a">
                        <div style="font-size:1.6rem;font-weight:700;color:#f39c12"><?= $totalUsers - $comWpp ?></div>
                        <div style="font-size:.72rem;color:#6e7485;margin-top:2px">Sem WhatsApp</div>
                    </div>
                    <div style="text-align:center;padding:14px;background:#0d0f14;border-radius:8px;border:1px solid #2a2f3a;grid-column:span 2">
                        <div style="font-size:1.6rem;font-weight:700;color:#f5b041"><?= number_format($totalEnvios) ?></div>
                        <div style="font-size:.72rem;color:#6e7485;margin-top:2px">Mensagens enviadas no total</div>
                    </div>
                </div>
...
<div class="wpp-grid-3">
        ...
        <div class="card-body">
            <form method="POST">
                ...
                <div class="wpp-form-group">
                    ...
                    <select name="destinatario" onchange="document.getElementById('selectUser').style.display=this.value==='individual'?'block':'none'">
    ...
    <div class="card">
        ...
        <div style="overflow-y:auto;max-height:620px">
            ...
            <div style="padding:40px;text-align:center;color:#6e7485">Nenhuma notificação ainda.</div>
            <?php endif; ?>
            ...
            <div class="notif-row">
                ...
                <div style="font-size:.76rem;color:#6e7485;margin-bottom:6px">
                    <?= htmlspecialchars(mb_substr($n['mensagem'], 0, 80)) ?>...
                </div>
                <div style="display:flex;justify-content:space-between;align-items:center">
                    <span style="font-size:.72rem;color:#6e7485">
...
<div class="wpp-grid-3">
    ...
    <div class="card">
        ...
        <div style="padding:40px;text-align:center;color:#6e7485">Nenhuma enquete ainda.</div>
        <?php endif; ?>
        <?php foreach ($enquetes as $eq):
            $resps  = $db->query("SELECT resposta, COUNT(*) as total FROM enquetes_respostas WHERE enquete_id={$eq['id']} GROUP BY resposta ORDER BY total DESC")->fetchAll();
            $totR   = array_sum(array_column($resps, 'total'));
        ...
        <div class="notif-row">
            ...
            <div style="font-size:.78rem;color:#6e7485;margin-bottom:10px"><?= htmlspecialchars($eq['pergunta']) ?></div>
            <?php foreach ($opcoes as $op):
                $qt  = 0;
                foreach ($resps as $r) { if ($r['resposta'] === $op) $qt = (int)$r['total']; }
                $pct = $totR > 0 ? round($qt / $totR * 100) : 0;
            ?>
            <div style="margin-bottom:7px">
                ...
                <div style="height:5px;background:#2a2f3a;border-radius:3px">
                    <div style="height:5px;background:#f39c12;border-radius:3px;width:<?= $pct ?>%"></div>
                </div>
            </div>
            <?php endforeach; ?>
            <div style="font-size:.7rem;color:#6e7485;margin-top:8px"><?= $totR ?> resposta(s) · <?= date('d/m/Y', strtotime($eq['criado_em'])) ?></div>
        </div>
        <?php endforeach; ?>
    </div>
</div>
        ...
        <table class="users-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="font-size:.72rem;color:#6e7485"><?= htmlspecialchars($log['erro_msg'] ?? '—') ?></td>
                </tr>
                <?php endforeach; ?>
                <?php if (empty($logs)): ?>
                <tr><td colspan="6" style="text-align:center;padding:30px;color:#6e7485">Nenhum envio registrado ainda.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
