...
document.addEventListener('DOMContentLoaded', function () {
  ...
  /* ── Abertura de Caixa ────────────────────────────────────── */
  var formAbrirCaixa = document.getElementById('formAbrirCaixa');
  if (formAbrirCaixa) {
    formAbrirCaixa.addEventListener('submit', async function (e) {
      ...
      try {
        var r = await fetch(BASE_URL + 'api/caixa.php', { method: 'POST', headers: { 'X-CSRF-Token': (typeof CSRF_TOKEN !== 'undefined' ? CSRF_TOKEN : '') }, body: new FormData(this) });
        var d = await r.json();
        if (d.success) {
        ...
        err.textContent = 'Erro de conexão: ' + ex.message;
        err.classList.remove('d-none');
        btn.disabled = false;
        btn.innerHTML = '<i class="ph-bold ph-lock-open me-2"></i>Abrir Caixa';
      }
...
async function salvarVenda(total, forma, mpData) {
  ...
  if (btnFin) btnFin.disabled = true;
  try {
    var r = await fetch(BASE_URL + 'api/venda.php', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': (typeof CSRF_TOKEN !== 'undefined' ? CSRF_TOKEN : '') }, body: JSON.stringify(payload) });
...
async function finalizarMercadoPago(total) {
  ...
  try {
    var r = await fetch(BASE_URL + 'api/mercadopago.php', {
      method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': (typeof CSRF_TOKEN !== 'undefined' ? CSRF_TOKEN : '') },
...
async function finalizarDinheiro(total) {
  var conf = await Swal.fire({
    icon: 'question', title: 'Confirmar Pagamento em Dinheiro',
    html: 'Total: <strong style="color:#f59e0b;font-size:1.3rem">' + fmtMoeda(total) + '</strong>',
    showCancelButton: true, confirmButtonText: '✅ Confirmar', cancelButtonText: 'Cancelar',
    confirmButtonColor: '#22c55e', background: '#1e2330', color: '#f0f2f7'
  });
  if (conf.isConfirmed) await salvarVenda(total, 'dinheiro', null);
}
...
function cancelarMercadoPago() {
  ...
  if (mpOrderId) { fetch(BASE_URL + 'api/mercadopago.php', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': (typeof CSRF_TOKEN !== 'undefined' ? CSRF_TOKEN : '') }, body: JSON.stringify({ action: 'cancelar', order_id: mpOrderId }) }).catch(function(){}); mpOrderId = null; }
...
async function confirmarFechamento() {
  ...
  try {
    var r = await fetch(BASE_URL + 'api/caixa.php', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': (typeof CSRF_TOKEN !== 'undefined' ? CSRF_TOKEN : '') }, body: JSON.stringify({ action: 'fechar', caixa_id: CAIXA_ID, saldo_contado: parseMoeda(s), observacoes: o }) });
    var d = await r.json();
