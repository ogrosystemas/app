...
db.on('populate', async () => {
  ...
  await db.servicos.bulkAdd([
    ...
    // PEDREIRO (inclui serviços pesados)
    { nome: 'Levantamento de alvenaria', tempoPadrao: 60, categoria: 'Alvenaria', profissaoId: profissoesIds.pedreiro, precoFixo: null },
    { nome: 'Assentamento de tijolos', tempoPadrao: 60, categoria: 'Alvenaria', profissaoId: profissoesIds.pedreiro, precoFixo: null },
...
db.version(4).upgrade(async (trans) => {
  try {
    // Apenas garantir que não quebre
...
db.version(5).upgrade(async (trans) => {
  try {
    // Verificar se a tabela profissoes existe
    const tabelas = await trans.db.tables.map(t => t.name);

    if (tabelas.includes('profissoes')) {
      // Desativar profissoes antigas se existirem
      const marteleteiro = await trans.table('profissoes').where('slug').equals('marteleteiro').first();
      if (marteleteiro) {
        await trans.table('profissoes').update(marteleteiro.id, { ativo: false });
      }

      const pintorPedreiro = await trans.table('profissoes').where('slug').equals('pintor-pedreiro').first();
      if (pintorPedreiro) {
        await trans.table('profissoes').update(pintorPedreiro.id, { ativo: false });
      }
    }
  } catch (error) {
    console.error('Upgrade error:', error);
