...
const VisualizarOrcamento = () => {
  ...
  const loadOrcamento = async () => {
    try {
      const budget = await db.orcamentos.get(parseInt(id));
      if (budget) {
        setOrcamento(budget);
...
Para aprovar ou solicitar alterações, entre em contato.
    ...
    const url = `https://wa.me/55${cliente.whatsapp.replace(/\D/g, '')}?text=${encodeURIComponent(message)}`;
  ...
  return (
    <div className="space-y-4 animate-fade-in pb-20">
          ...
          {orcamento.itens.map((item, idx) => (
            <div key={idx} className="flex justify-between items-start">
              <div>
                ...
                <p className="text-xs text-slate-500">{formatarTempo(item.tempo)} • {item.dificuldade}</p>
              </div>
      ...
      {/* Actions */}
      <div className="fixed bottom-0 left-0 right-0 lg:left-72 p-4 bg-white border-t border-slate-200">
