...
import { ArrowLeft, Send, Camera as CameraIcon, CheckCircle, XCircle, Clock, Calendar, Trash2, CheckSquare } from 'lucide-react';
...
const VisualizarOrcamento = ({ onBack, id }) => {
  ...
  const loadOrcamento = async (orcId) => {
    ...
    } catch (err) { console.error(err); showToast('Erro ao carregar orçamento', 'error'); }
  ...
  const updateStatus = async (newStatus) => {
    ...
    showToast(`Status alterado para ${newStatus === 'aprovado' ? 'Aprovado' : newStatus === 'pendente' ? 'Pendente' : 'Recusado'}`, 'success');
  };
  ...
  const concluirOrcamento = async () => {
    ...
    if (orcamento.status === 'concluido') {
      showToast('Orçamento já concluído', 'info');
      setShowConcluirModal(false);
      return;
    }
    // Lança no caixa
  ...
  const deleteOrcamento = async () => {
    if (!orcamento) return;
    // Remove possível lançamento no caixa associado
    const lanc = await db.caixa.where('orcamentoId').equals(orcamento.id).first();
    if (lanc && confirm('Este orçamento tem um lançamento no caixa. Deseja excluí-lo também?')) {
      await db.caixa.delete(lanc.id);
    }
  ...
  const sendWhatsApp = () => {
    ...
    setEnviando(true);
    const dataVenc = orcamento.dataVencimento ? new Date(orcamento.dataVencimento).toLocaleDateString('pt-BR') : 'Não informada';
    const mensagem = `*ORÇAMENTO MÃO DE OBRA PRO*%0A*Nº:* ${orcamento.id}%0A*Cliente:* ${cliente.nome}%0A*Data:* ${new Date(orcamento.data).toLocaleDateString('pt-BR')}%0A*Válido até:* ${dataVenc}%0A%0A*SERVIÇOS:*%0A${orcamento.itens.map(i => `✓ ${i.nome} x${i.quantidade||1} - ${formatarMoeda(i.precoTotal||i.preco)}`).join('%0A')}%0A%0A*DESLOCAMENTO:* ${formatarMoeda(orcamento.taxaDeslocamento)}%0A${orcamento.desconto ? `*DESCONTO:* ${orcamento.desconto>0 ? formatarMoeda(orcamento.desconto) : Math.abs(orcamento.desconto)+'%'}` : ''}%0A*TOTAL:* ${formatarMoeda(orcamento.total)}`;
    window.location.href = `https://wa.me/55${cliente.whatsapp.replace(/\D/g,'')}?text=${mensagem}`;
    setEnviando(false);
  };
    ...
    <div className="space-y-4 pb-32">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          {podeConcluir && <button onClick={() => setShowConcluirModal(true)} className="bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center gap-2"><CheckSquare size={18} /> Concluir</button>}
      ...
      <div className={`rounded-xl p-4 text-center cursor-pointer ${orcamento.status==='concluido'?'bg-green-100':orcamento.status==='aprovado'?'bg-green-50':orcamento.status==='pendente'?'bg-yellow-50':'bg-red-50'}`} onClick={() => setShowStatusModal(true)}>
        <div className="flex justify-center gap-2">{orcamento.status==='concluido'&&<CheckCircle className="text-green-700"/>}{orcamento.status==='aprovado'&&<CheckCircle className="text-green-600"/>}{orcamento.status==='pendente'&&<Clock className="text-yellow-600"/>}{orcamento.status==='recusado'&&<XCircle className="text-red-600"/>}
        <span className="font-semibold">{orcamento.status==='concluido'?'Concluído':orcamento.status==='aprovado'?'Aprovado':orcamento.status==='pendente'?'Pendente':'Recusado'}</span></div>
      </div>
      ...
      <div className="bg-white rounded-xl border"><div className="p-4 bg-slate-50 border-b"><h3 className="font-semibold flex items-center gap-2"><Calendar size={18}/> Validade</h3></div><div className="p-4"><div className="flex justify-between"><span>Válido até:</span><span className={isVencido?'text-red-600':'text-green-600'}>{dataVenc}{isVencido&&' (Vencido)'}</span></div></div></div>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t"><button onClick={sendWhatsApp} disabled={enviando} className="w-full bg-green-600 text-white py-3 rounded-lg font-semibold flex justify-center gap-2"><Send size={20}/> Enviar via WhatsApp</button></div>

      {showStatusModal && <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50"><div className="bg-white rounded-xl max-w-md w-full p-6"><h3 className="text-xl font-bold mb-4">Alterar Status</h3><div className="space-y-3"><button onClick={()=>updateStatus('pendente')} className="w-full p-3 border-2 border-yellow-500 bg-yellow-50 rounded flex items-center justify-center gap-2"><Clock size={20}/> Pendente</button><button onClick={()=>updateStatus('aprovado')} className="w-full p-3 border-2 border-green-500 bg-green-50 rounded flex items-center justify-center gap-2"><CheckCircle size={20}/> Aprovado</button><button onClick={()=>updateStatus('recusado')} className="w-full p-3 border-2 border-red-500 bg-red-50 rounded flex items-center justify-center gap-2"><XCircle size={20}/> Recusado</button></div><button onClick={()=>setShowStatusModal(false)} className="w-full mt-4 border py-2 rounded">Cancelar</button></div></div>}
      <ConfirmModal isOpen={showDeleteModal} onClose={()=>setShowDeleteModal(false)} onConfirm={deleteOrcamento} title="Excluir Orçamento" message={`Tem certeza?`} confirmText="Excluir" cancelText="Cancelar" />
      <ConfirmModal isOpen={showConcluirModal} onClose={()=>setShowConcluirModal(false)} onConfirm={concluirOrcamento} title="Concluir Orçamento" message={`Lançar ${formatarMoeda(orcamento.total)} no caixa?`} confirmText="Concluir" cancelText="Cancelar" />
    </div>
  );
};
