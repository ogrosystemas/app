...
const ConfiguracoesPage = () => {
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      <div className="overflow-x-auto -mx-4 px-4"><div className="flex gap-2 border-b min-w-max">{menus.map(m=>{const Icon=m.icon; return <button key={m.id} onClick={()=>setActiveMenu(m.id)} className={`px-4 py-2 font-semibold whitespace-nowrap ${activeMenu===m.id?'text-blue-600 border-b-2 border-blue-600':'text-slate-500'}`}><div className="flex items-center gap-2"><Icon size={18}/><span>{m.label}</span></div></button>})}</div></div>
      {activeMenu === 'config' && (<>
