  ...
  return new Promise((resolve, reject) => {
    ...
    req.onupgradeneeded = (e) => {
      ...
      if (old < 1) {
        ...
        if (profissoes) profissoes.createIndex('slug', 'slug', { unique: true });

        const clientes = store('clientes', 'id', true);
        if (clientes) clientes.createIndex('nome', 'nome', { unique: false });
        ...
        if (fotos) fotos.createIndex('orcamentoId', 'orcamentoId', { unique: false });
      ...
      // v2 — adiciona store pagamentos
      if (old < 2) {
        const pagamentos = store('pagamentos', 'id', true);
        if (pagamentos) {
          pagamentos.createIndex('orcamentoId', 'orcamentoId', { unique: false });
          pagamentos.createIndex('data',        'data',        { unique: false });
        }
      }
    };
...
export async function seedIfEmpty() {
  ...
  const ids = {};
  for (const p of profissoes) ids[p.slug] = await add('profissoes', p);
