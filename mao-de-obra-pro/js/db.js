...
const SCHEMA = {
  profissoes:  { keyPath: 'id', autoIncrement: true },
  clientes:    { keyPath: 'id', autoIncrement: true },
  servicos:    { keyPath: 'id', autoIncrement: true },
  orcamentos:  { keyPath: 'id', autoIncrement: true },
  fotos:       { keyPath: 'id', autoIncrement: true },
  config:      { keyPath: 'chave' },
  caixa:       { keyPath: 'id', autoIncrement: true },
};

const INDEXES = {
  profissoes:  [['slug', 'slug', { unique: true }]],
  clientes:    [['nome', 'nome', {}]],
  servicos:    [['profissaoId', 'profissaoId', {}], ['categoria', 'categoria', {}]],
  orcamentos:  [['clienteId', 'clienteId', {}], ['profissaoId', 'profissaoId', {}], ['status', 'status', {}], ['data', 'data', {}]],
  fotos:       [['orcamentoId', 'orcamentoId', {}]],
  caixa:       [['data', 'data', {}], ['orcamentoId', 'orcamentoId', {}]],
};
...
export function openDB() {
  ...
  return new Promise((resolve, reject) => {
    ...
    req.onupgradeneeded = (e) => {
      ...
      for (const [store, opts] of Object.entries(SCHEMA)) {
        if (!db.objectStoreNames.contains(store)) {
          const os = db.createObjectStore(store, opts);
          for (const [, idxName, idxKey, idxOpts] of (INDEXES[store] || [])) {
            os.createIndex(idxName, idxKey, idxOpts);
          }
        }
      }
    };
...
export async function getAll(store, indexName = null, query = null) {
  ...
  return promisify(query ? source.getAll(query) : source.getAll());
}
...
export async function seedIfEmpty() {
  ...
  const profissoes = [
    ...
    { slug: 'encanador',    nome: 'Encanador',       icone: 'bi-droplet-fill',          riscoBase: 1.1, custoFerramental: 200, descricao: 'Foco em tempo de estanqueidade e reparo', ativo: 1 },
    ...
    { slug: 'pintor',       nome: 'Pintor',          icone: 'bi-brush-fill',            riscoBase: 1.0, custoFerramental: 150, descricao: 'Preparação de superfícies, pintura interna/externa', ativo: 1 },
  ...
  const servicos = [
    ...
    { nome: 'Desentupimento de vaso sanitário',  tempoPadrao: 45,  categoria: 'Hidráulica',    profissaoId: ids.encanador,    precoFixo: 100.00 },
