...
export async function producaoPage() {
  const [todos, pedidos, materiais] = await Promise.all([
    db.producao  ? db.producao.orderBy('id').reverse().toArray()  : [],
    db.pedidos   ? db.pedidos.toArray()   : [],
    db.materiais ? db.materiais.toArray() : [],
  ]);

  const producao   = todos.filter(i => !i.arquivado);
  const arquivo    = todos.filter(i =>  i.arquivado);
  ...
  return `
    <section class="pb-4">
      ...
      <!-- ATIVAS -->
      ${!producao.length
        ? emptyState({ icon:'hammer', title:'Nenhuma faca em produção', description:'Inicie uma produção para acompanhar cada etapa.' })
        : `<div class="grid-stack">${producao.map(i => renderCard(i, false)).join('')}</div>`
      }

      <!-- ARQUIVO -->
      ${arquivo.length ? `
        <div style="margin-top:32px">
          <button id="toggleArquivoProducao" style="
...
async function openConsumoModal(producaoId, producaoNome, materiaisPrevistos, onConfirm) {
  ...
  if (!todosMateriaisDB.length) { await onConfirm([]); return; }
    ...
    content: `
      <div style="margin-bottom:16px">
        <p style="font-size:14px;color:var(--muted);line-height:1.6">
          Confirme os materiais usados em <strong style="color:var(--text)">${producaoNome}</strong>.
          Os valores pré-preenchidos são os reservados.
        </p>
      </div>
          ...
          return `
            <div style="display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid var(--border)">
              ...
              <div style="display:flex;align-items:center;gap:6px;flex-shrink:0">
                <input type="number" class="consumo-input"
  ...
  document.getElementById('consumoForm').addEventListener('submit', async (e) => {
    e.preventDefault();
    const consumos = [];
    for (const input of document.querySelectorAll('.consumo-input')) {
      const qtd = parseFloat(input.value) || 0;
      if (qtd <= 0) continue;
      const id    = Number(input.dataset.id);
      const atual = parseFloat(input.dataset.atual) || 0;
      if (qtd > atual) {
        showToast({ type:'error', message:`Quantidade maior que estoque disponível (${input.dataset.unidade}).` });
        return;
      }
...
async function openProducaoModal(existing = null) {
  const [pedidosAbertos, materiais] = await Promise.all([
    db.pedidos
      ? db.pedidos.filter(p => !p.arquivado && (p.status === STATUS_PEDIDO.ABERTO || p.status === STATUS_PEDIDO.EM_PRODUCAO)).toArray()
      : [],
    db.materiais ? db.materiais.toArray() : [],
  ]);
  ...
  const reservaAtual = Object.fromEntries((existing?.materiaisPrevistos || []).map(p => [p.materialId, p.qtdPrevista]));
      ...
      <form id="producaoForm" class="grid-stack">
        ...
        ` : `
          <div style="border-top:1px solid var(--border);padding-top:14px">
            <p style="font-size:13px;color:var(--muted)">Cadastre materiais (Aço e Cabo) para reservar aqui.</p>
          </div>
        `}
  ...
  document.getElementById('producaoForm').addEventListener('submit', async (e) => {
    ...
    document.querySelectorAll('.reserva-input').forEach(input => {
      const qtd = parseFloat(input.value) || 0;
      if (qtd > 0) materiaisPrevistos.push({ materialId: Number(input.dataset.materialId), qtdPrevista: qtd });
    });
    ...
    const data = {
      ...
      status, progresso: STATUS_PROGRESS[status] || 0,
    ...
    if (existing) {
      ...
      if (data.pedidoId) await db.pedidos.update(data.pedidoId, { status: STATUS_PEDIDO.EM_PRODUCAO });
...
window.addEventListener('click', async (e) => {
  ...
  // Toggle arquivo
  if (e.target.id === 'toggleArquivoProducao' || e.target.closest('#toggleArquivoProducao')) {
    const lista   = document.getElementById('arquivoProducaoLista');
    const chevron = document.getElementById('arquivoProducaoChevron');
  ...
  if (e.target.closest('.advance-status-btn') && id) {
    ...
    if (newStatus === STATUS_PRODUCAO.FINALIZADA) { finalizarProducao(id); return; }
  ...
  if (e.target.closest('.archive-prod-btn') && id) {
    await db.producao.update(id, { arquivado: true, arquivadoEm: new Date().toISOString() });
    showToast({ message: 'Produção arquivada.' });
    setTimeout(() => navigate('producao'), 300);
    return;
  }

  if (e.target.closest('.unarchive-prod-btn') && id) {
    await db.producao.update(id, { arquivado: false, arquivadoEm: null });
    showToast({ message: 'Produção desarquivada.' });
    setTimeout(() => navigate('producao'), 300);
    return;
  }

  if (e.target.closest('.delete-prod-btn') && id) {
    if (!confirm('Excluir permanentemente esta produção?')) return;
