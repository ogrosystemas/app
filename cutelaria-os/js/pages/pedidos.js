...
export async function pedidosPage() {
  const pedidos = db.pedidos ? await db.pedidos.orderBy('id').reverse().toArray() : [];

  const abertos   = pedidos.filter(p => p.status === STATUS_PEDIDO.ABERTO).length;
  const emProd    = pedidos.filter(p => p.status === STATUS_PEDIDO.EM_PRODUCAO).length;
  const concluidos= pedidos.filter(p => p.status === STATUS_PEDIDO.CONCLUIDO).length;
  const totalReceber = pedidos.filter(p => p.status !== STATUS_PEDIDO.CANCELADO && p.status !== STATUS_PEDIDO.CONCLUIDO)
  ...
  return `
    <section class="pb-4">
      ...
      <!-- LISTA -->
      ${!pedidos.length ? emptyState({
        icon: 'shopping-bag',
        title: 'Nenhum pedido ainda',
        description: 'Crie pedidos para organizar suas vendas e acompanhar prazos.'
      }) : `
...
function openPedidoModal(existing = null) {
  const title = existing ? 'Editar Pedido' : 'Novo Pedido';
  openModal({
    title,
    size: 'md',
    content: `
...
window.addEventListener('click', async (e) => {
  ...
  if (e.target.closest('.conclude-pedido-btn') && id) {
    ...
    // Lançar receita automaticamente se ainda não foi
