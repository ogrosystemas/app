...
<nav class="os-navbar">
  ...
  <div class="nav-item-top">
    <button class="nav-link-top <?= in_array($current,['os.php','orcamentos.php','clientes.php','form_laudo.php','laudo.php'])?'active':'' ?>">
    ...
    <div class="nav-dropdown">
      ...
      <a href="<?= $baseUrl ?>/modules/laudo/laudo.php" class="<?= in_array($current,['laudo.php','form_laudo.php'])?'active':'' ?>">
        <i class="ph-bold ph-clipboard-text"></i> Relatório Técnico
      </a>
    </div>
  </div>
...
<?php
// Banner de aviso de licença (trial expirando ou expirado)
if (function_exists('licenca_status') && isset($db)) {
    $_licS = licenca_status($db);
    if ($_licS['status'] === 'trial' && $_licS['dias_restantes'] <= 5) {
        $cor = $_licS['dias_restantes'] <= 2 ? '#ef4444' : '#f59e0b';
        $bg  = $_licS['dias_restantes'] <= 2 ? 'rgba(239,68,68,.1)' : 'rgba(245,158,11,.1)';
        $bd  = $_licS['dias_restantes'] <= 2 ? 'rgba(239,68,68,.3)' : 'rgba(245,158,11,.3)';
        echo '<div style="margin:8px 16px 0;padding:10px 16px;background:' . $bg . ';border:1px solid ' . $bd . ';border-radius:10px;display:flex;align-items:center;gap:10px;font-size:.82rem;color:' . $cor . '">';
        echo '<i class="ph-bold ph-warning-circle"></i>';
        echo '<span><strong>Trial expira em ' . $_licS['dias_restantes'] . ' dia(s).</strong> ';
        echo '<a href="' . BASE_URL . '/modules/configuracoes/configuracoes.php" style="color:' . $cor . ';font-weight:600">Ativar licença →</a></span>';
        echo '</div>';
    } elseif ($_licS['status'] === 'ativa' && $_licS['dias_restantes'] <= 10) {
        echo '<div style="margin:8px 16px 0;padding:10px 16px;background:rgba(245,158,11,.1);border:1px solid rgba(245,158,11,.3);border-radius:10px;display:flex;align-items:center;gap:10px;font-size:.82rem;color:#f59e0b">';
        echo '<i class="ph-bold ph-clock-countdown"></i>';
        echo '<span><strong>Licença expira em ' . $_licS['dias_restantes'] . ' dia(s).</strong> ';
        echo '<a href="' . BASE_URL . '/modules/configuracoes/configuracoes.php" style="color:#f59e0b;font-weight:600">Renovar →</a></span>';
        echo '</div>';
    }
}
?>
